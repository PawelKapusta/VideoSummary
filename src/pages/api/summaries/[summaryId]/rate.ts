...
export const POST: APIRoute = async ({ params, request, locals }) => {
  ...
  try {
    ...
    if (!validation.success) {
      const duration = performance.now() - startTime;
      errorLogger.validationError(new Error('Rate summary validation failed'), undefined, undefined, { endpoint: `/api/summaries/${summaryId}/rate`, method: 'POST' });
      securityLogger.apiAccess({ method: 'POST', path: `/api/summaries/${summaryId}/rate`, statusCode: 400 });
      performanceLogger.apiResponseTime('POST', `/api/summaries/${summaryId}/rate`, duration, 400);
    ...
    if (checkError || !summaryCheck) {
      const duration = performance.now() - startTime;
      errorLogger.appError(new Error('Summary not found'), { endpoint: `/api/summaries/${summaryId}/rate`, method: 'POST' });
    ...
    if (upsertError || !ratingData) {
      const duration = performance.now() - startTime;
      errorLogger.appError(upsertError || new Error('Failed to upsert rating'), { endpoint: `/api/summaries/${summaryId}/rate`, method: 'POST' });
    ...
    const duration = performance.now() - startTime;
    errorLogger.appError(error instanceof Error ? error : new Error(String(error)), { endpoint: `/api/summaries/${summaryId}/rate`, method: 'POST' });
    securityLogger.apiAccess({ method: 'POST', path: `/api/summaries/${summaryId}/rate`, statusCode: 500 });
    performanceLogger.apiResponseTime('POST', `/api/summaries/${summaryId}/rate`, duration, 500);
