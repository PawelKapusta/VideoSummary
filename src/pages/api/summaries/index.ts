  ...
  try {
    ...
    if (bulkInProgress) {
      ...
      const errorResponse: ApiError = {
        error: {
          ...
          message:
            "Cannot generate individual summaries while bulk generation is in progress. Please wait for bulk generation to complete.",
        },
      };
    ...
    if (error instanceof Error) {
      if (error.message === "VIDEO_TOO_LONG") {
        ...
        error.message === "VIDEO_PRIVATE" ||
        error.message === "YT_CAPTION_LIST_AUTH_REQUIRED" ||
        error.message === "YT_CAPTION_DOWNLOAD_AUTH_REQUIRED"
      ) {
        statusCode = 404;
        errorCode = "VIDEO_PRIVATE";
        message = "Video is private or unavailable";
      } else if (
        error.message === "NO_SUBTITLES" ||
        error.message === "TRANSCRIPT_NOT_AVAILABLE" ||
        error.message === "TRANSCRIPT_EMPTY"
      ) {
        statusCode = 422;
...
export const GET: APIRoute = async ({ locals, url }) => {
  ...
  try {
    ...
    const result: PaginatedResponse<SummaryWithVideo> = await listSummaries(supabase, userId, {
      ...filters,
