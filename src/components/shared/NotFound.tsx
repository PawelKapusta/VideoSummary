...
import { Home, ArrowLeft, Search, Mail, PlayCircle, MonitorOff, Youtube, FileSearch, Sparkles } from 'lucide-react';
...
const NotFound: React.FC<NotFoundProps> = ({
    ...
    const [elements, setElements] = React.useState<{ size: string, top: string, left: string, delay: string, opacity: number, color: string, duration: string, type: 'play' | 'pixel' }[]>([]);
            ...
            <div className="text-center space-y-12 max-w-5xl px-4 relative z-10 w-full pt-10">
                ...
                <div className="space-y-8 max-w-2xl mx-auto">
                    <div className="space-y-4">
                        <p className="text-slate-700 text-xl sm:text-2xl leading-relaxed font-semibold">
                            {message}
                        </p>
                ...
                <div className="pt-20 grid grid-cols-1 md:grid-cols-3 gap-6 text-left border-t border-slate-100">
                    {[
                        ...
                        >
                            <div className={`w-12 h-12 rounded-xl ${item.bgColor} flex items-center justify-center mb-6 transition-transform group-hover:scale-110 shadow-sm`}>
