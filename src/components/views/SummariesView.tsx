...
class ErrorBoundary extends React.Component<{ children: React.ReactNode }, { hasError: boolean; error?: Error }> {
  ...
  render() {
    if (this.state.hasError) {
      return <ErrorState error={this.state.error} onRetry={() => window.location.reload()} compact={false} />;
    }
...
const SummariesContent: React.FC = () => {
  ...
  const emptyMessage =
    flattenedData.length === 0 && hasActiveFilters
  ...
  return (
    <ErrorBoundary>
      <div data-testid="summaries-view" className="container mx-auto p-4 pt-12 pb-12">
        ...
        {flattenedData.length === 0 && !isLoading ? (
          hasActiveFilters ? (
            <div className="flex flex-col items-center justify-center py-16 px-8 max-w-2xl mx-auto">
              ...
              <h2 className="text-3xl font-bold text-gray-900 mb-4 text-center">No summaries match your filters</h2>
