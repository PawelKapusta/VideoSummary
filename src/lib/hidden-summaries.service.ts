...
export async function hideSummary(
  ...
  const { error: insertError } = await supabase.from("hidden_summaries").insert({
    user_id: userId,
...
export async function unhideAllSummaries(
  ...
  const { count, error } = await supabase.from("hidden_summaries").delete({ count: "exact" }).eq("user_id", userId);
