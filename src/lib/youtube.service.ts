...
export async function fetchYouTubeVideoMetadata(
  ...
  try {
    ...
    const [videoData, captionsData] = await Promise.all([videoResponse.json(), captionsResponse.json()]);
    ...
    const hasSubtitles = captionsResponse.ok && captionsData.items && captionsData.items.length > 0;
