 ...
 */
export function createSecureYouTubeFetchOptions(
...
export function validateYouTubeApiUrl(url: string): boolean {
  try {
    ...
    if (!allowedHosts.includes(parsedUrl.hostname)) {
      securityLogger.logSecurityEvent(
        'suspicious_activity',
        'Invalid YouTube API URL detected',
        {
          service: 'youtube_api',
    ...
    securityLogger.logSecurityEvent(
      'suspicious_activity',
      'Malformed URL in YouTube API request',
      {
        service: 'youtube_api',
 ...
 * @param userId - Optional user ID for rate limiting
 */
export async function secureYouTubeFetch(
  url: string,
  runtimeEnv?: RuntimeEnv,
  userId?: string
): Promise<Response> {
